...
"""
Minimal CARLA No-Rendering 2D Map Visualizer

- Shows a 2D top-down map.
- Draws vehicles, walkers, traffic lights, and speed limits.
...
class HelpText(object):
    def __init__(self, font, width, height):
        help_lines = [
            "Minimal CARLA 2D Map Visualizer",
            "",
            "Mouse wheel : zoom",
            "Left-drag   : pan",
            "",
        ...
        self.font = font
        self.dim = (420, len(help_lines) * 22 + 12)
...
class HUD(object):
    ...
    def render_vehicles_ids(self, vehicle_id_surface, list_actors, world_to_pixel):
        ...
        if self.show_actor_ids:
            ...
            for actor, transform in list_actors:
                x, y = world_to_pixel(transform.location)
                ...
                if actor.attributes.get('role_name', '').startswith('controlled_agent'):
                    color = COLOR_SCARLET_RED_0

                font_surface = self._header_font.render(str(actor.id), True, color)
                rect = font_surface.get_rect(center=(x, y))
                vehicle_id_surface.blit(font_surface, rect)
...
class MapImage(object):
    def __init__(self, carla_world, carla_map, pixels_per_meter,
        ...
        dirname = os.path.join("cache", "no_rendering_mode_simple")
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def get_lane_markings(lane_marking_type, lane_marking_color, waypoints, sign):
            margin = 0.25
            marking_1 = [world_to_pixel(lateral_shift(w.transform, sign * w.lane_width * 0.5))
                         for w in waypoints]
            if lane_marking_type in (carla.LaneMarkingType.Broken,
        ...
        def draw_traffic_signs(surface, font_surface, actor, color=COLOR_ALUMINIUM_2,
                               trigger_color=COLOR_PLUM_0):
            transform = actor.get_transform()
            waypoint = carla_map.get_waypoint(transform.location)
            angle = -waypoint.transform.rotation.yaw - 90.0
            font_surface_rot = pygame.transform.rotate(font_surface, angle)
            pixel_pos = world_to_pixel(waypoint.transform.location)
            offset = font_surface_rot.get_rect(center=(pixel_pos[0], pixel_pos[1]))
            surface.blit(font_surface_rot, offset)
...
class World(object):
    ...
    def render(self, display):
        ...
        self.traffic_light_surfaces.rotozoom(0.0, self.map_image.scale)
        ...
        clipping_rect = pygame.Rect(-translation_offset[0] - center_offset[0],
                                    -translation_offset[1],
                                    self._hud.dim[0], self._hud.dim[1])
        self.clip_surfaces(clipping_rect)
        Util.blits(self.result_surface, surfaces)

        display.blit(self.result_surface,
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 4:
                    ...
                    if self.wheel_offset >= 1.5:
                        self.wheel_offset = 1.5
                elif event.button == 5:
                    self.wheel_offset -= self.wheel_amount
                    if self.wheel_offset <= 0.05:
                        self.wheel_offset = 0.05
...
def main():
    argparser = argparse.ArgumentParser(
        description='Minimal CARLA 2D Map Visualizer (No Rendering)')
